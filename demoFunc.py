...
def main():
    ...
    for round_idx in range(NUM_ROUNDS):
        ...
        for epoch in range(NUM_EPOCH):
            ...
            try:
                for batch_idx, (inputs_batch, targets_p_batch, targets_fault_batch, targets_p_orig_batch,
                                inputs_orig_batch) in enumerate(
                        train_loader):
                    # 打印调试信息
                    ...
                    if epoch == 0 and batch_idx == 0:
                        ...
                        print(
                            f"mdot_A_orig 值范围: [{mdot_A_orig_batch.min().item():.6f}, {mdot_A_orig_batch.max().item():.6f}]")
                        print(
                            f"标准化后 mdot_A 值范围: [{inputs_batch[:, :, 2].min().item():.6f}, {inputs_batch[:, :, 2].max().item():.6f}]")
                    ...
                    for val_batch_idx, (inputs_val_batch, targets_p_val_batch, targets_fault_val_batch,
                        ...
                        with torch.set_grad_enabled(True):
                            ...
                            if epoch == 0 and val_batch_idx == 0:
                                ...
                                print(
                                    f"mdot_A_orig_val 值范围: [{mdot_A_orig_val_batch.min().item():.6f}, {mdot_A_orig_val_batch.max().item():.6f}]")
                    ...
                    for test_batch_idx, (inputs_test_batch, targets_p_test_batch, targets_fault_test_batch,
                        ...
                        with torch.set_grad_enabled(True):
                            ...
                            if epoch == 0 and test_batch_idx == 0:
                                ...
                                print(
                                    f"mdot_A_orig_test 值范围: [{mdot_A_orig_test_batch.min().item():.6f}, {mdot_A_orig_test_batch.max().item():.6f}]")
        ...
        if all_p_pred_test and all_targets_p_test:
            ...
            if len(all_fault_logits_test) > 0 and len(all_targets_fault_test) > 0:
                ...
                cls_metrics = calculate_classification_metrics(
                    all_targets_fault_test, 
                    all_fault_pred_test,
                    1 / (1 + np.exp(-all_fault_logits_test)) if isinstance(all_fault_logits_test, np.ndarray) else all_fault_logits_test
                )
            ...
            print('\n=== 分类指标 ===')
            print('准确率 (Accuracy): {:.4f}'.format(cls_metrics['accuracy']))
            print('精确率 (Precision): {:.4f}'.format(cls_metrics['precision']))
            print('召回率 (Recall): {:.4f}'.format(cls_metrics['recall']))
            print('F1分数: {:.4f}'.format(cls_metrics['f1']))
            print('特异性 (Specificity): {:.4f}'.format(cls_metrics['specificity']))
            if 'auc' in cls_metrics:
                print('AUC: {:.4f}'.format(cls_metrics['auc']))
            
            cm = cls_metrics['confusion_matrix']
            print('\n混淆矩阵:')
            print(f'真阴性(TN): {cm["tn"]}, 假阳性(FP): {cm["fp"]}')
            print(f'假阴性(FN): {cm["fn"]}, 真阳性(TP): {cm["tp"]}')
    ...
    if all_losses['train'] and all(isinstance(losses, list) and losses for losses in all_losses['train']):
        ...
        plt.title(f'Loss Function Trend (Average, {NUM_ROUNDS} Rounds)')
        plt.legend()
        plt.grid(True)
