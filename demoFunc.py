...
class My_loss(nn.Module):
    def __init__(self, physics_weight=0.4, cls_weight=0.6):  # 添加物理损失权重和分类损失权重参数
        super().__init__()
    ...
    def forward(self, p_pred, p_t_pred, fault_logit, targets_p, targets_fault, mdot_A, V, 
        ...
        if torch.rand(1).item() < 0.01:  # 随机打印1%的批次避免日志过多
            ...
            print(f"损失函数 - p_pred范围(反标准化): [{p_min_val:.4f}, {p_max_val:.4f}], targets_p范围(标准化): [{t_min_val:.4f}, {t_max_val:.4f}]")
        ...
        if targets_p_orig is not None:
            ...
            if mean_p is not None and std_p is not None:
                ...
                if torch.rand(1).item() < 0.01:  # 随机打印
                    ...
                    print(f"损失函数 - targets_p_orig范围(反标准化计算): [{targets_denorm_min:.4f}, {targets_denorm_max:.4f}], mean_p={mean_p.item():.4f}, std_p={std_p.item():.4f}")
            else:
...
class MultiTaskPINN(nn.Module):
    ...
    def forward(self, x):
        ...
        try:
            ...
            print(f"梯度计算错误: {e}")
            P_t = torch.zeros_like(t, device=device)
...
class SequenceDataset(Dataset):
    def __init__(self, X, y_p, y_fault, seq_len, y_p_orig):
        """
        序列数据集，用于创建滑动窗口数据
...
def main():
    ...
    # 1. 数据加载和预处理
    print("1. 加载和预处理数据...")
    data = pd.read_csv('D:\cyh\project\PINN-Battery-Prognostics-main\PINN-Battery-Prognostics-main\combined_all_425_2.csv')
    ...
    # 训练集
    train_set = SequenceDataset(X_train, y_p_train, y_fault_train, SEQ_LEN, y_p_train_orig)
    print(f"训练集实际窗口数: {len(train_set)}")
    
    # 验证集
    val_set = SequenceDataset(X_val, y_p_val, y_fault_val, SEQ_LEN, y_p_val_orig)
    
    # 测试集
    test_set = SequenceDataset(X_test, y_p_test, y_fault_test, SEQ_LEN, y_p_test_orig)
    ...
    for round_idx in range(NUM_ROUNDS):
        ...
        for epoch in range(NUM_EPOCH):
            ...
            try:
                for batch_idx, (inputs_batch, targets_p_batch, targets_fault_batch, targets_p_orig_batch) in enumerate(train_loader):
                    # 打印调试信息
            ...
            try:
                # 使用验证数据加载器进行批量处理
                with torch.no_grad():
                    for val_batch_idx, (inputs_val_batch, targets_p_val_batch, targets_fault_val_batch, targets_p_orig_val_batch) in enumerate(val_loader):
                        # 移动数据到设备
                        ...
                        # 获取mdot_A
                        mdot_A_val_batch = inputs_val_batch[:, :, 2]  # [batch, seq_len]
                        ...
                        with torch.set_grad_enabled(True):
                            ...
                            val_loss_batch = criterion(
                                ...
                                mdot_A_val_batch,  # [batch, seq_len]
                                V=2 * 1e-4,
            ...
            if num_val_batches > 0:
                ...
                if best_val_loss > avg_val_loss:
                    ...
                    torch.save(model.state_dict(), 'D:\cyh\project\PINN-Battery-Prognostics-main\PINN-Battery-Prognostics-main\my_model_weights.pth')
                    no_improvement_count = 0
                else:
            ...
            try:
                # 使用测试数据加载器进行批量处理
                with torch.no_grad():
                    for test_batch_idx, (inputs_test_batch, targets_p_test_batch, targets_fault_test_batch, targets_p_orig_test_batch) in enumerate(test_loader):
                        # 移动数据到设备
                        ...
                        # 获取mdot_A
                        mdot_A_test_batch = inputs_test_batch[:, :, 2]  # [batch, seq_len]
                        ...
                        test_loss_batch = criterion(
                            ...
                            mdot_A_test_batch,  # [batch, seq_len]
                            V=2 * 1e-4,
        ...
        # 加载最佳模型
        model.load_state_dict(torch.load('D:\cyh\project\PINN-Battery-Prognostics-main\PINN-Battery-Prognostics-main\my_model_weights.pth'))
        model.eval()
        ...
        if all_p_pred_test and all_targets_p_test:
            ...
            if all_p_pred_test.shape != all_targets_p_orig_test.shape:
                print(f"警告: 维度不匹配，调整中。 p_pred: {all_p_pred_test.shape}, targets_orig: {all_targets_p_orig_test.shape}")
                # 确保两个数组有相同的形状
            ...
            results = {
                ...
                'fault_prob': 1 / (1 + np.exp(-all_fault_logits_test)).flatten() if isinstance(all_fault_logits_test, np.ndarray) else all_fault_logits_test.flatten(),
                'metrics': {
                    'rmse': rmse,
                    'mae': mae,
                    'mape': mape,
    ...
    if len(all_p_pred_test) > 0 and len(all_targets_p_orig_test) > 0:
        ...
        results = {
            ...
            'fault_prob': 1 / (1 + np.exp(-all_fault_logits_test)).flatten() if len(all_fault_logits_test.shape) > 0 else np.array([]),
            'metrics': {
                'rmse': rmse,
                'mae': mae,
                'mape': mape,
